#!/bin/bash
# ============================================================================
# Uncertainty re-run — model: QRF
# quantile spread; strongest error-ranker in existing results
# ============================================================================
...
#
#   sbatch --account=<acct> --partition=medium --array=0-71%6 \
#          unc_qrf.sh
#
...
#   sbatch --account=<acct> --partition=medium --array=3,17,40 unc_qrf.sh
# ============================================================================
#SBATCH --job-name=unc_qrf
#SBATCH --output=unc_qrf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
cd /private/tmp/claude-501/-Users-apunt-repos-qsar-qm-models/e1d07839-2376-4458-891b-01d0a9e0433b/scratchpad/stub/repo
. /private/tmp/claude-501/-Users-apunt-repos-qsar-qm-models/e1d07839-2376-4458-891b-01d0a9e0433b/scratchpad/stub/repo/setup.sh
cd tests
...
OUT="results/uncertainty_rerun/qrf__${ds}__${rep_slug}__${st}"

if [ -z "${SLURM_JOB_PARTITION:-}" ]; then
    echo "ERROR: no partition. Submit with --partition=medium (see RUNBOOK step 4)."; exit 2
fi

echo "=== task $i: model=QRF dataset=$ds rep=$rep strategy=$st"
echo "=== out: $OUT"
echo "=== started: $(date)"
