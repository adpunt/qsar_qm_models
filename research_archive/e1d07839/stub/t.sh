...
#   sbatch --account=<acct> --partition=medium --array=3,17,40 unc_gp.sh
# ============================================================================
#SBATCH --job-name=unc_gp
#SBATCH --output=unc_gp_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="results/uncertainty_rerun/gp__${ds}__${rep_slug}__${st}"

if [ -z "${SLURM_JOB_PARTITION:-}" ]; then
    echo "ERROR: no partition. Submit with --partition=medium (see RUNBOOK step 4)."; exit 2
fi

echo "=== task $SLURM_ARRAY_TASK_ID: model=GP dataset=$ds rep=$rep strategy=$st"
echo "=== out: $OUT"
echo "=== started: $(date)"
