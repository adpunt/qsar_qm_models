#!/bin/bash
# ============================================================================
# Uncertainty re-run — model: BNN-Full
...
#   sbatch --account=<acct> --partition=medium --array=3,17,40 unc_bnn_full.sh
# ============================================================================
#SBATCH --job-name=unc_bnn_full
#SBATCH --output=unc_bnn_full_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python "/data/stat-cadd/scat9264/qsar_qm_models/scripts/check_environment.py" --validation-models "BNN-Full" || {
    echo "ERROR: this interpreter cannot build BNN-Full. See above."
    exit 2
}
...
OUT="results/uncertainty_rerun/bnn_full__${ds}__${rep_slug}__${cond}"

if [ -z "${SLURM_JOB_PARTITION:-}" ]; then
    echo "ERROR: no partition. Submit with --partition=medium (see RUNBOOK step 4)."; exit 2
fi

echo "=== task $i: model=BNN-Full dataset=$ds rep=$rep condition=$cond"
echo "=== out: $OUT"
echo "=== started: $(date)"
