...
python "/data/stat-cadd/scat9264/qsar_qm_models/scripts/check_environment.py" --validation-models "QRF" || {
    echo "ERROR: this interpreter cannot build QRF. See above."
    exit 2
}
...
OUT="results/uncertainty_rerun/qrf__${ds}__${rep_slug}__${cond}"

if [ -z "${SLURM_JOB_PARTITION:-}" ]; then
    echo "ERROR: no partition. Submit with --partition=medium (see RUNBOOK step 4)."; exit 2
fi

echo "=== task $i: model=QRF dataset=$ds rep=$rep condition=$cond"
echo "=== out: $OUT"
echo "=== started: $(date)"
