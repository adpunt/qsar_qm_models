#!/bin/bash
# ============================================================================
# Uncertainty re-run — model: MLP-VBLL-Full
...
#   sbatch --account=<acct> --partition=medium --array=3,17,40 unc_mlp_vbll_full.sh
# ============================================================================
#SBATCH --job-name=unc_mlp_vbll_full
#SBATCH --output=unc_mlp_vbll_full_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python "/data/stat-cadd/scat9264/qsar_qm_models/scripts/check_environment.py" --validation-models "MLP-VBLL-Full" || {
    echo "ERROR: this interpreter cannot build MLP-VBLL-Full. See above."
    exit 2
}
...
OUT="results/uncertainty_rerun/mlp_vbll_full__${ds}__${rep_slug}__${cond}"

if [ -z "${SLURM_JOB_PARTITION:-}" ]; then
    echo "ERROR: no partition. Submit with --partition=medium (see RUNBOOK step 4)."; exit 2
fi

echo "=== task $i: model=MLP-VBLL-Full dataset=$ds rep=$rep condition=$cond"
echo "=== out: $OUT"
echo "=== started: $(date)"

python -u alternative_data_noise_robustness.py \
    --datasets "$ds" \
    --models "MLP-VBLL-Full" \
    --reps "$rep" \
    --conditions "$cond" \
    --unc-conditions all \
    --oof-folds 5 \
