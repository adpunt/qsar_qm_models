...
MODELS = {
    ...
    'QRF':            (1, 8,  '64G', 36, 'first on BOTH measures on all six representations, and the cheapest: tracks its error 0.25-0.35, truth inside 1 sd 0.70-0.83 against a target of 0.68'),
    'NGBoost':        (1, 8,  '64G', 47, 'second on four representations of six (0.09-0.30), mildly overconfident (0.53-0.66). Expensive -- 7.4x the forest on the screen -- and kept because it is the noise-robust model the study highlights'),
    'GP':             (1, 8,  '64G', 47, 'the only non-tree model that shows anything, and it depends on the representation: 0.28 on PDV, 0.19 on ChemBERTa, 0.06 on ECFP4. gauche ExactGP, RBF kernel'),
    'VBLL-Full':      (2, 8,  '64G', 47, 'the variational network. Badly overconfident -- truth inside 1 sd 0.27-0.51 against a target of 0.68 -- which is itself the finding. On ALL THREE representations from 2026-09-01: the ChemBERTa restriction is lifted'),
    ...
    # theirs from.
    'BNN-Full-MVE':     (2, 8,  '64G', 47, 'a Bayesian network with a VARIANCE HEAD -- the literature flagship case, and the only network whose aleatoric term varies per molecule'),
    'MLP-BNN-Full-MVE': (2, 8,  '64G', 47, 'the same variance head on the NN-beta base, so the finding does not rest on one architecture'),
}
