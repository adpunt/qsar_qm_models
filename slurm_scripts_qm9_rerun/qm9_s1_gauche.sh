#!/bin/bash
# ============================================================================
# QM9 re-run — stage 1 — model: gauche
# Tanimoto GP. Only defined on BINARY fingerprints, so ecfp4/sns only. This is the RBF-vs-Tanimoto head-to-head; the figure script gives it its own colour and marker and labels it GP
# ============================================================================
# Array task -> (noise condition, representation).
#   3 conditions x 2 representations = 6 tasks
...
#   sbatch --account=<acct> --partition=medium --array=0-5%5 qm9_s1_gauche.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_gauche.sh
# ============================================================================
#SBATCH --job-name=qm91_gauche
#SBATCH --output=qm91_gauche_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models gauche || {
    echo "ERROR: this interpreter cannot build model 'gauche'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_gauche.csv"

echo "=== task $i: model=gauche condition=$cond rep=$rep"
...
python -u process_and_train.py -d QM9 -t homo_lumo_gap \
    -m gauche --kernel tanimoto -u True \
    -r "$rep" \
    --noise-level $LEVELS \
    --dose-units spread \
    $NOISE_FLAGS \
