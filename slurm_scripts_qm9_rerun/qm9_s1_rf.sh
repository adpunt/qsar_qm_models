...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_rf.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_rf.sh
# ============================================================================
#SBATCH --job-name=qm91_rf
#SBATCH --output=qm91_rf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models rf || {
    echo "ERROR: this interpreter cannot build model 'rf'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_rf.csv"

echo "=== task $i: model=rf condition=$cond rep=$rep"
