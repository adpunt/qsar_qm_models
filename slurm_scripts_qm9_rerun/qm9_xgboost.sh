...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_xgboost.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_xgboost.sh
# ============================================================================
#SBATCH --job-name=qm9_xgboost
#SBATCH --output=qm9_xgboost_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_xgboost.csv"

echo "=== task $i: model=xgboost strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
