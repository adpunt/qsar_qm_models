...
#   sbatch --account=<acct> --partition=medium --array=0-35%5 qm9_dnn.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_dnn.sh
# ============================================================================
#SBATCH --job-name=qm9_dnn
#SBATCH --output=qm9_dnn_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_dnn.csv"

echo "=== task $i: model=dnn strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
