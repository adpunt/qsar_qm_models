#!/bin/bash
# ============================================================================
# QM9 re-run — stage 1 — model: qrf
# not in the ANOVA (rho 0.996 with rf) but the best error-ranker
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_qrf.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_qrf.sh
# ============================================================================
#SBATCH --job-name=qm91_qrf
#SBATCH --output=qm91_qrf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models qrf || {
    echo "ERROR: this interpreter cannot build model 'qrf'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_qrf.csv"

echo "=== task $i: model=qrf condition=$cond rep=$rep"
