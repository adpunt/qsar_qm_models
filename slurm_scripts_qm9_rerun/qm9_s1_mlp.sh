...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_mlp.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_mlp.sh
# ============================================================================
#SBATCH --job-name=qm91_mlp
#SBATCH --output=qm91_mlp_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models mlp || {
    echo "ERROR: this interpreter cannot build model 'mlp'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_mlp.csv"

echo "=== task $i: model=mlp condition=$cond rep=$rep"
