#!/bin/bash
# ============================================================================
# QM9 re-run — stage 1 — model: svm
# RBF kernel on every representation
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_svm.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_svm.sh
# ============================================================================
#SBATCH --job-name=qm91_svm
#SBATCH --output=qm91_svm_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models svm || {
    echo "ERROR: this interpreter cannot build model 'svm'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_svm.csv"

echo "=== task $i: model=svm condition=$cond rep=$rep"
