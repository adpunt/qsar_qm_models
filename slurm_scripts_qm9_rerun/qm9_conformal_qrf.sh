#!/bin/bash
# ============================================================================
# QM9 re-run — model: conformal_qrf
# conformal wrapper
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_conformal_qrf.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_conformal_qrf.sh
# ============================================================================
#SBATCH --job-name=qm9_conformal_qrf
#SBATCH --output=qm9_conformal_qrf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_conformal_qrf.csv"

echo "=== task $i: model=conformal_qrf strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"

python -u process_and_train.py -d QM9 -t homo_lumo_gap \
    -m conformal --cp-base-model qrf -u True \
    -r "$rep" \
