#!/bin/bash
# ============================================================================
# QM9 re-run — model: dnn_bnn_variational
# pre-VBLL variational (was identical to last-layer, a bug)
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_dnn_bnn_variational.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_dnn_bnn_variational.sh
# ============================================================================
#SBATCH --job-name=qm9_dnn_bnn_variational
#SBATCH --output=qm9_dnn_bnn_variational_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_dnn_bnn_variational.csv"

echo "=== task $i: model=dnn_bnn_variational strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"

python -u process_and_train.py -d QM9 -t homo_lumo_gap \
    -m dnn --bayesian-transformation variational -u True \
    -r "$rep" \
