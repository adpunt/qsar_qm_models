#!/bin/bash
# ============================================================================
# QM9 re-run — stage 1 — model: mlp_bnn_full
# BNN-beta
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_mlp_bnn_full.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_mlp_bnn_full.sh
# ============================================================================
#SBATCH --job-name=qm91_mlp_bnn_full
#SBATCH --output=qm91_mlp_bnn_full_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models mlp_bnn_full || {
    echo "ERROR: this interpreter cannot build model 'mlp_bnn_full'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_mlp_bnn_full.csv"

echo "=== task $i: model=mlp_bnn_full condition=$cond rep=$rep"
...
python -u process_and_train.py -d QM9 -t homo_lumo_gap \
    -m mlp --bayesian-transformation full -u True \
    -r "$rep" \
    --noise-level $LEVELS \
    --dose-units spread \
    $NOISE_FLAGS \
