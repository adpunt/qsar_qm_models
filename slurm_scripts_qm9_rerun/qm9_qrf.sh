...
#   sbatch --account=<acct> --partition=medium --array=0-35%5 qm9_qrf.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_qrf.sh
# ============================================================================
#SBATCH --job-name=qm9_qrf
#SBATCH --output=qm9_qrf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_qrf.csv"

echo "=== task $i: model=qrf strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
