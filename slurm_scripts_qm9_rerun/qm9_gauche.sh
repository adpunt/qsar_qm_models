...
#   6 strategies x 2 representations = 12 tasks
...
#   sbatch --account=<acct> --partition=medium --array=0-11%5 qm9_gauche.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_gauche.sh
# ============================================================================
#SBATCH --job-name=qm9_gauche
#SBATCH --output=qm9_gauche_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_gauche.csv"

echo "=== task $i: model=gauche strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
