...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_mlp.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_mlp.sh
# ============================================================================
#SBATCH --job-name=qm9_mlp
#SBATCH --output=qm9_mlp_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_mlp.csv"

echo "=== task $i: model=mlp strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
