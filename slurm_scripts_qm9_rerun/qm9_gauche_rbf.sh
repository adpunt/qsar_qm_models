...
#   sbatch --account=<acct> --partition=medium --array=0-35%5 qm9_gauche_rbf.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_gauche_rbf.sh
# ============================================================================
#SBATCH --job-name=qm9_gauche_rbf
#SBATCH --output=qm9_gauche_rbf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_gauche_rbf.csv"

echo "=== task $i: model=gauche_rbf strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
