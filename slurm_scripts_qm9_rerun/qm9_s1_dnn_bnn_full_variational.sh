#!/bin/bash
# ============================================================================
# QM9 re-run — stage 1 — model: dnn_bnn_full_variational
# VBLL-alpha (figure script reads this as dnn_vbll)
# ============================================================================
# Array task -> (noise condition, representation).
#   3 conditions x 6 representations = 18 tasks
#   each task = 7 noise levels x 9 replicate(s) = 63 training runs
#
# Conditions in this stage: gaussian grouped_wider grouped_shifted
# Replicates: 9, numbered 1..9. The replicate seed
...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_dnn_bnn_full_variational.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_dnn_bnn_full_variational.sh
# ============================================================================
#SBATCH --job-name=qm91_dnn_bnn_full_variational
#SBATCH --output=qm91_dnn_bnn_full_variational_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models dnn_bnn_full_variational || {
    echo "ERROR: this interpreter cannot build model 'dnn_bnn_full_variational'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_dnn_bnn_full_variational.csv"

echo "=== task $i: model=dnn_bnn_full_variational condition=$cond rep=$rep"
echo "=== noise: $NOISE_FLAGS"
echo "=== levels: $LEVELS"
echo "=== replicates: 9 starting at 1"
echo "=== out: $OUT"
echo "=== started: $(date)"

python -u process_and_train.py -d QM9 -t homo_lumo_gap \
    -m dnn --bayesian-transformation full_variational -u True \
    -r "$rep" \
    --noise-level $LEVELS \
    --dose-units spread \
    $NOISE_FLAGS \
