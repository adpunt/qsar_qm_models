...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_svm.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_svm.sh
# ============================================================================
#SBATCH --job-name=qm9_svm
#SBATCH --output=qm9_svm_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_svm.csv"

echo "=== task $i: model=svm strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
