#!/bin/bash
# ============================================================================
# QM9 re-run — model: conformal_dnn
# conformal wrapper
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_conformal_dnn.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_conformal_dnn.sh
# ============================================================================
#SBATCH --job-name=qm9_conformal_dnn
#SBATCH --output=qm9_conformal_dnn_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_conformal_dnn.csv"

echo "=== task $i: model=conformal_dnn strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"

python -u process_and_train.py -d QM9 -t homo_lumo_gap \
    -m conformal --cp-base-model dnn -u True \
    -r "$rep" \
