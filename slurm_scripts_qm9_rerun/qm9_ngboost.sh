...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_ngboost.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_ngboost.sh
# ============================================================================
#SBATCH --job-name=qm9_ngboost
#SBATCH --output=qm9_ngboost_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_ngboost.csv"

echo "=== task $i: model=ngboost strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
