...
So this drives the real script over a fixture in three states:
  ...
  3. a row that was never a copy disagrees  -> DISAGREES and STOPS, exit 1
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if 'STOPPING' not in p.stdout:
            failures.append('a genuine disagreement did not stop the copy')
    ...
    print('PASS — copies are refreshed when the reference is re-run, a genuine '
          'disagreement still stops the copy, and both are reported.')
    return 0
