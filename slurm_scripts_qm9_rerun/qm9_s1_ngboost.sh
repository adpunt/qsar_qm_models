#!/bin/bash
# ============================================================================
# QM9 re-run — stage 1 — model: ngboost
# slowest tree model; emits per-molecule uncertainty
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_ngboost.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_ngboost.sh
# ============================================================================
#SBATCH --job-name=qm91_ngboost
#SBATCH --output=qm91_ngboost_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models ngboost || {
    echo "ERROR: this interpreter cannot build model 'ngboost'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_ngboost.csv"

echo "=== task $i: model=ngboost condition=$cond rep=$rep"
