...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_dnn.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_dnn.sh
# ============================================================================
#SBATCH --job-name=qm91_dnn
#SBATCH --output=qm91_dnn_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models dnn || {
    echo "ERROR: this interpreter cannot build model 'dnn'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_dnn.csv"

echo "=== task $i: model=dnn condition=$cond rep=$rep"
