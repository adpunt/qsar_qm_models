#!/bin/bash
# ============================================================================
# QM9 re-run — stage 1 — model: gauche_rbf
# RBF GP on EVERY rep, so the GP can finally enter the cross-rep ANOVA
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_gauche_rbf.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_gauche_rbf.sh
# ============================================================================
#SBATCH --job-name=qm91_gauche_rbf
#SBATCH --output=qm91_gauche_rbf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models gauche_rbf || {
    echo "ERROR: this interpreter cannot build model 'gauche_rbf'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_gauche_rbf.csv"

echo "=== task $i: model=gauche_rbf condition=$cond rep=$rep"
