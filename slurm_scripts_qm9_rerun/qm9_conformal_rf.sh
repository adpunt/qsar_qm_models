#!/bin/bash
# ============================================================================
# QM9 re-run — model: conformal_rf
# conformal wrapper (rho > 0.99 with rf)
# ============================================================================
...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_conformal_rf.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_conformal_rf.sh
# ============================================================================
#SBATCH --job-name=qm9_conformal_rf
#SBATCH --output=qm9_conformal_rf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_conformal_rf.csv"

echo "=== task $i: model=conformal_rf strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"

python -u process_and_train.py -d QM9 -t homo_lumo_gap \
    -m conformal --cp-base-model rf -u True \
    -r "$rep" \
