...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_dnn_bnn_full.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_dnn_bnn_full.sh
# ============================================================================
#SBATCH --job-name=qm9_dnn_bnn_full
#SBATCH --output=qm9_dnn_bnn_full_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_dnn_bnn_full.csv"

echo "=== task $i: model=dnn_bnn_full strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"
