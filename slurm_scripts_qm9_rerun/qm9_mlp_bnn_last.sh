...
#   sbatch --account=<acct> --partition=medium --array=0-29%5 qm9_mlp_bnn_last.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_mlp_bnn_last.sh
# ============================================================================
#SBATCH --job-name=qm9_mlp_bnn_last
#SBATCH --output=qm9_mlp_bnn_last_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUT="../results/anova_${tag}_${rep}_mlp_bnn_last.csv"

echo "=== task $i: model=mlp_bnn_last strategy=$strat rep=$rep"
echo "=== out: $OUT"
echo "=== started: $(date)"

python -u process_and_train.py -d QM9 -t homo_lumo_gap \
    -m mlp --bayesian-transformation last -u True \
    -r "$rep" \
