...
#   sbatch --account=<acct> --partition=medium --array=0-17%5 qm9_s1_lgb.sh
#
# Resubmit only failed indices:
#   sbatch --account=<acct> --partition=medium --array=3,17 qm9_s1_lgb.sh
# ============================================================================
#SBATCH --job-name=qm91_lgb
#SBATCH --output=qm91_lgb_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python check_environment.py --models lgb || {
    echo "ERROR: this interpreter cannot build model 'lgb'. See the output above."
    exit 2
}
...
OUT="../results/anova_${cond}_${rep}_lgb.csv"

echo "=== task $i: model=lgb condition=$cond rep=$rep"
