#!/bin/bash
#SBATCH --job-name=val_XGBoost_mhg_gnn_pretrained_cac
...
python "$QSAR_DIR/scripts/check_environment.py" --validation-models XGBoost || {
    echo "ERROR: this interpreter cannot build XGBoost. See above."
    exit 2
}
...
echo "Done: XGBoost x MHG-GNN-pretrained x caco2"
