#!/bin/bash
#SBATCH --job-name=val_LightGBM_mhg_gnn_pretrained_ca
...
python "$QSAR_DIR/scripts/check_environment.py" --validation-models LightGBM || {
    echo "ERROR: this interpreter cannot build LightGBM. See above."
    exit 2
}
...
python alternative_data_noise_robustness.py \
    ...
    --results-root results/validation_rerun/mhg_gnn_pretrained_caco2

echo "Done: LightGBM x MHG-GNN-pretrained x caco2"
