...
def every_targeting_name_the_rows_carry_parses():
    """The name written on a results row must be one the program accepts.

    `--noise-targeting` took `grouped_wide` while every row, manifest and figure
    said `grouped_wider`, so the obvious thing to type -- the name read off a row
    -- died at argparse before the value ever reached the injector. The Rust side
    accepting both spellings is not enough on its own; this driver refuses an
    unlisted choice first (NOISE_DESIGN.md 6.2b).

    The names come from `condition_names.json`, which is where they are settled.
    """
    import json
    parser = parser_of_process_and_train()
    table = json.load(open(os.path.join(HERE, "..", "condition_names.json")))
    wanted = sorted({row["targeting"] for row in table["names"]})

    bad = []
    for targeting in wanted:
        err = io.StringIO()
        argv = ["-m", "rf", "-r", "ecfp4", "-d", "QM9",
                "--noise-targeting", targeting]
        try:
            with contextlib.redirect_stderr(err), \
                    contextlib.redirect_stdout(io.StringIO()):
                parser.parse_args(argv)
        except SystemExit:
            bad.append(f"--noise-targeting {targeting}: "
                       f"{err.getvalue().strip().splitlines()[-1]}")
    assert not bad, (
        f"{len(bad)} of {len(wanted)} targeting names that appear on results "
        f"rows are refused by this program:\n    " + "\n    ".join(bad))
    print(f"    {len(wanted)} targeting names, every one accepted")
...
def main():
    ...
    results = [
        ...
        check("every targeting name the rows carry parses",
              every_targeting_name_the_rows_carry_parses),
    ]
    if not all(results):
