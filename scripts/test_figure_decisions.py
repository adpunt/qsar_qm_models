...
def the_anova_leaves_the_variance_head_models_out():
    ...
    They train under a different likelihood from the model they are a variant
    of, so they would move the MODEL term of a decomposition that asks how much
    of the accuracy spread is the model.
    """
    import figlib_metrics as M
    frame = pd.DataFrame([
        {'model': m, 'rep': r, 'condition': 'gaussian', 'replicate': i,
         'auc_norm': 0.9}
        for m in ('rf', 'svm', 'dnn_vbll_hetero', 'mlp_bnn_full_mve',
                  'het_gp_rbf')
        for r in ('ecfp4', 'pdv') for i in range(10)])
    kept = M.drop_variant_models(frame, where='a test')
    assert set(kept['model']) == {'rf', 'svm'}, sorted(set(kept['model']))
    assert len(kept) == 40, len(kept)
    # and it is only the ANOVA -- the frame it was given is untouched
    assert len(frame) == 100
    print('    the five heteroscedastic variants are out of the ANOVA and in '
          'every other figure')
