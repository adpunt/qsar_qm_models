...
REPRESENTATION_COLORS = {
    ...
    'mhggnn': '#CC79A7',  # Pink/magenta for MHGGNN pretrained embeddings
}
...
def load_screening_results(results_dir="../results"):
    ...
    # Load both phase0c_screen_*.csv and phase0_mhggnn_*.csv files
...
def create_figure1_global_landscape(df, metrics_df, output_dir):
    ...
    ax_a.set_title('A. Top 20 Most Noise-Robust Configurations', fontsize=11, fontweight='bold', pad=12)
    ...
    ax_b.set_title('B. Bottom 20 Least Robust Configurations', fontsize=11, fontweight='bold', pad=12)
    ...
    heatmap_data = metrics_df.pivot_table(
        index='model', columns='representation', values='retention_pct', aggfunc='mean'
    )
    ...
    ax_c.set_title('C. Global Heatmap: Retention % at High Noise', fontsize=10, fontweight='bold', pad=10)
...
def create_figure2_representation_effects(df, metrics_df, output_dir):
    ...
    if len(valid_data_robust) > 0:
        parts = ax_b.violinplot(valid_data_robust, positions=range(len(valid_reps_robust)), widths=0.7, showmeans=True, showmedians=True)
        ...
        ax_b.set_xticklabels([format_representation(r) for r in valid_reps_robust], rotation=45, ha='right')
    ...
    # PANEL C: Noise sensitivity ranking
    ...
    ax_c.bar(range(len(noise_difficulty)), noise_difficulty['difficulty'], color=colors, alpha=0.8, edgecolor='black', linewidth=0.8)
    ax_c.set_xticks(range(len(noise_difficulty)))
    ax_c.set_xticklabels([format_representation(r) for r in noise_difficulty['representation']], rotation=45, ha='right')
...
def create_supplementary_s1(metrics_df, output_dir):
    ...
    ax.set_title('Supplementary S1: Baseline vs Noise Retention', fontsize=11, fontweight='bold', pad=15)
...
def create_supplementary_s2(metrics_df, output_dir):
    ...
    ax_a = fig.add_subplot(gs[0, 0])
    ax_a.hist(metrics_df['nsi_r2'].dropna(), bins=30, color='#3498db', alpha=0.7, edgecolor='black', linewidth=0.8)
    ax_a.axvline(metrics_df['nsi_r2'].median(), color='red', linestyle='--', linewidth=2, label=f'Median = {metrics_df["nsi_r2"].median():.4f}')
    ...
    parts = ax_b.violinplot([metrics_df[metrics_df['representation'] == rep]['nsi_r2'].dropna() for rep in rep_order],
    ...
    ax_b.set_xticklabels([format_representation(r) for r in rep_order], rotation=45, ha='right')
    ...
    ax_c.hist(metrics_df['nsi_rmse'].dropna(), bins=30, color='#e74c3c', alpha=0.7, edgecolor='black', linewidth=0.8)
    ax_c.axvline(metrics_df['nsi_rmse'].median(), color='blue', linestyle='--', linewidth=2, label=f'Median = {metrics_df["nsi_rmse"].median():.4f}')
    ...
    parts = ax_d.violinplot([metrics_df[metrics_df['model'] == model]['nsi_r2'].dropna() for model in model_order],
    ...
    ax_d.set_xticklabels([format_model(m) for m in model_order], rotation=45, ha='right', fontsize=7)
...
def create_summary_tables(metrics_df, output_dir):
    ...
    table1 = metrics_df.nlargest(20, 'robustness_score')[
        ['model', 'representation', 'baseline_r2', 'r2_high', 'retention_pct', 'nsi_r2', 'robustness_score']
    ].copy()
    table1.columns = ['Model', 'Representation', 'Baseline R²', 'R² (high noise)', 'Retention (%)', 'NSI (R²)', 'Robustness Score']
...
def perform_noise_robustness_anova(metrics_df, output_dir):
    ...
    print(f"\nData for ANOVA: {len(analysis_df)} observations")
    ...
    all_results = {}
    output_text = ["="*80, "ANOVA VARIANCE DECOMPOSITION - NOISE ROBUSTNESS METRICS", "="*80, ""]
    ...
    with open(output_path, 'w') as f:
        f.write('\n'.join(output_text))
    print(f"✓ Saved noise robustness ANOVA to {output_path}")
...
def main(results_dir="../results"):
    ...
    for idx, (_, row) in enumerate(metrics_df.nlargest(10, 'robustness_score').iterrows(), 1):
        print(f"  {idx}. {format_model(row['model'])}/{format_representation(row['representation'])}: "
              f"Score={row['robustness_score']:.3f}, R²₀={row['baseline_r2']:.3f}, Retention={row['retention_pct']:.1f}%")
