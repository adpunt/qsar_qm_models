...
def main():
    ...
    # so a request for one dataset and model is one command, not a re-derivation.
    want = None
    if '--show' in sys.argv:
    ...
    if want:
        ds_want, _, m_want = want.partition('/')
        out, cur_ds, keep = [], None, False
        for line in lines:
            if line.startswith('## '):
                cur_ds = line[3:].strip()
                keep = (cur_ds == ds_want) and not m_want
                if cur_ds == ds_want:
                    out.append(line)
                continue
            if line.startswith('### '):
        ...
        print('\n'.join(out) if len(out) > 1 else
              f'no table for {want!r}. Datasets: {DATASETS}; '
