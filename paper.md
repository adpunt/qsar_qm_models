...
% TODO [DATA PENDING]: Add NGBoost to LogD validation once jobs 11365616-19 complete.
%   Also add BNN Full/Last if validation jobs submitted. Update model count in text.
% TODO [FILTERED — MUST DOCUMENT IN PAPER]:
%   Validation baseline R² threshold = 0.3 (vs 0.6 for QM9). The following were filtered:
%   1. PDV representation catastrophic failures (baseline R² << 0):
%      - DNN+PDV on hERG-Ki: R² = -2096 (complete divergence)
%      - GP(Gauche)+PDV on all 3 datasets: R² = -12 (hERG), -0.14 (LogD), -0.10 (Caco2)
%      - DNN+PDV on LogD: R² = -0.02, DNN+PDV on Caco2: R² = -0.002
%      - SVM+PDV on hERG-Ki: R² = -0.02, SVM+PDV on LogD: R² = 0.007
%      → PDV fails systematically on external datasets. Discuss: PDV descriptors may be
%        QM9-specific (quantum-chemical properties) and not transferable to drug-like datasets.
%   2. GP (Gauche) only available with PDV representation (kernel requires molecular descriptors).
%      GP is missing from ECFP4/MHG-GNN/SNS on all datasets — not a gap, by design.
%   3. Extreme NDS values (|NDS| > 2.0) set to N/A in heatmaps (DNN+PDV divergence artifacts).
%   4. herg_fluid/ directory excluded: classification dataset (hERG-FLuID), not regression.
%   5. openadmet_caco2/ and openadmet_logd/ excluded: older runs without scaffold CV folds,
%      superseded by caco2/ and logd/ which have 5-fold scaffold CV.
% TODO [TEXT]: Baseline R² ranges to report in paper:
%   - LogD: R² = 0.63–0.82 (strong baselines, well above threshold)
%   - hERG-Ki: R² = 0.40–0.56 (marginal, above 0.3 threshold but weaker than QM9)
%   - Caco2: R² = 0.37–0.62 (marginal, many configs near threshold)
%   → Note in paper: weaker baselines on hERG-Ki and Caco2 mean NDS is noisier.
% TODO [FIGURE]: Verify 3-panel heatmap appearance after regeneration with all 3 datasets.
\begin{figure*}[htbp]
\centering
