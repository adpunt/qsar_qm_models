...
To ensure meaningful robustness comparisons, we restricted analysis to configurations achieving baseline R$^2 > 0.6$, excluding 63 configurations where poor clean-data performance would produce misleadingly shallow degradation slopes (Supplementary Table~\ref{tab:excluded_configs}).
...
NGBoost exemplifies this link: it produces the strongest noise tracking and achieves both the best robustness and calibration (Table~\ref{tab:uncertainty_metrics}). At the other extreme, QRF produces uncertainty that is essentially unresponsive to noise despite strong error-uncertainty correlation---its uncertainty captures model variance but not data quality, mapping directly onto its poor robustness.
...
Uncertainty decomposition into epistemic and aleatoric components reveals the mechanism behind this noise responsiveness (Figure~\ref{fig:uncertainty_combined}B). In the GP (Gauche), which naturally separates signal variance (epistemic) from observation noise (aleatoric), aleatoric uncertainty rises steeply with $\sigma$ while epistemic uncertainty remains flat---the model correctly attributes injected noise to data quality rather than to model inadequacy. BNNs lack an explicit observation noise term, so their total uncertainty is entirely epistemic: they detect noise through increased weight posterior variance rather than through a dedicated noise channel. This architectural difference explains why BNN uncertainty tracks noise less sharply than the GP's ($\rho = 0.32$--$0.34$ for full BNNs versus $0.33$ for Gauche in total, but Gauche's aleatoric component alone tracks noise at $\rho > 0.5$). The VBLL variants, which learn an explicit observation noise parameter on the final layer, should in principle bridge this gap---providing a dedicated aleatoric channel within a neural network architecture.

These uncertainty patterns are not specific to a single noise type or representation: the uncertainty--noise correlation structure holds across all six strategies and four representations (Supplementary Table~\ref{tab:supp_uncertainty_by_strategy_rep}). The practical implication is that only models whose uncertainty is ``honest''---responsive to actual data quality, and specifically whose aleatoric component tracks label noise---provide the kind of noise resistance that generalizes.
...
Across all six noise strategies, model rankings were highly concordant ($W = 0.957$), and the relative severity of noise strategies was uniform across architectures---the noise structure, not the model, determines relative damage. However, robustness rankings did not transfer from QM9 to smaller experimental datasets ($r = -0.30$, $p = 0.51$). Critically, whether findings transfer depends on the \emph{mechanism}: SVM's inherent robustness generalizes to new domains, while XGBoost's dataset-specific robustness does not.
