#!/bin/bash
# ============================================================================
# Hyperparameter sweep: ngboost on 6 representation(s).
#
...
#
# Wall clock is 59h, from the MEASURED seconds for this model's slowest
# representation (chemberta, 5443s per fit) x 13 fits x 3.0
...
#   sbatch --account=stat-cadd --partition=medium --array=0-5%6 tune_ngboost.sh
#
...
# ============================================================================
#SBATCH --job-name=tune_ngboost
#SBATCH --output=tune_ngboost_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python scripts/check_environment.py --models ngboost || {
    echo "ERROR: this interpreter cannot build model 'ngboost'."
    exit 2
}
...
echo "=== task $i: model=ngboost rep=$rep"
