#!/bin/bash
# ============================================================================
# Hyperparameter sweep: rf on 6 representation(s).
#
...
#
# Wall clock is 5h, from the MEASURED seconds for this model's slowest
# representation (chemberta, 377s per fit) x 13 fits x 3.0
...
#   sbatch --account=stat-cadd --partition=medium --array=0-5%6 tune_rf.sh
#
...
# ============================================================================
#SBATCH --job-name=tune_rf
#SBATCH --output=tune_rf_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python scripts/check_environment.py --models rf || {
    echo "ERROR: this interpreter cannot build model 'rf'."
    exit 2
}
