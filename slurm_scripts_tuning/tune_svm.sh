#!/bin/bash
# ============================================================================
# Hyperparameter sweep: svm on 6 representation(s).
#
...
# representation (ecfp4, 135s per fit) x 13 fits x 3.0
...
#   sbatch --account=stat-cadd --partition=medium --array=0-5%6 tune_svm.sh
#
...
# ============================================================================
#SBATCH --job-name=tune_svm
#SBATCH --output=tune_svm_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python scripts/check_environment.py --models svm || {
    echo "ERROR: this interpreter cannot build model 'svm'."
    exit 2
}
