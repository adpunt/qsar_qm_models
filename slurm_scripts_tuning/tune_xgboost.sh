#!/bin/bash
# ============================================================================
# Hyperparameter sweep: xgboost on 6 representation(s).
#
...
#
# Wall clock is 2h, from the MEASURED seconds for this model's slowest
# representation (mhggnn, 149s per fit) x 13 fits x 3.0
...
#   sbatch --account=stat-cadd --partition=medium --array=0-5%6 tune_xgboost.sh
#
...
#SBATCH --output=tune_xgboost_%A_%a.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python scripts/check_environment.py --models xgboost || {
    echo "ERROR: this interpreter cannot build model 'xgboost'."
    exit 2
}
...
echo "=== task $i: model=xgboost rep=$rep"
