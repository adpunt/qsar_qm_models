...
def add_replicate_table_row(table, replicate_table, model_name, column='Model'):
    ...
    table = table._append(pd.Series([model_name], index=[column])
                         ._append(replicate_table[replicate_table[column] == model_name].mean(axis=0)), ignore_index=True)
...
def add_cluster_data(table, name, prefix, data, tag, size):
    ...
    return table._append({name: prefix + str(tag), 'Active': active, 'Inactive': inactive, 'Total': total,
...
def validate_teachers(teacher_models, test_data, params):
    ...
    numeric_mcc = pd.to_numeric(teacher_ev_table['MCC'], errors='coerce')
    average_teacher_table = pd.concat([
        average_teacher_table,
        pd.DataFrame({'Model': ['T-mean'], 'MCC': numeric_mcc.iloc[1:].mean()}),
        pd.DataFrame({'Model': ['T-max'], 'MCC': numeric_mcc.iloc[1:].max()}),
        pd.DataFrame({'Model': ['T-min'], 'MCC': numeric_mcc.iloc[1:].min()}),
        pd.DataFrame({'Model': ['T-std'], 'MCC': numeric_mcc.iloc[1:].std()})
    ], ignore_index=True)
...
"""
    Create the a new federated data initial table
---
...
"""
    Calculate non-euclidean distances between x and all other molecules in the data
---
                  x : query points, array of fingerprints
    distance_metric : distance metric for similarity calculation (Tanimoto, Dice, or Cosine)
        n_neighbors : number of nearest neighbors to classify
"""

def calculate_similarity(x, distance_metric, n_neighbors):
    num_samples = len(x)
    distances = np.zeros(shape=(num_samples, n_neighbors))
    indices = np.zeros(shape=(num_samples, n_neighbors), dtype=int)

    # Convert each fingerprint in the ndarray to RDKit fingerprint object
    x_fps = [DataStructs.CreateFromBitString(''.join(map(str, fp))) for fp in x]

    for i, base_fp in enumerate(tqdm(x_fps)):
        # Create a Python list of RDKit fingerprint objects
        x_fps_list = [DataStructs.CreateFromBitString(''.join(map(str, fp))) for fp in x]
        
        # Bulk similarity calculation with BulkTanimotoSimilarity
        if distance_metric == 'tanimoto':
            scores = DataStructs.BulkTanimotoSimilarity(base_fp, x_fps_list)
        elif distance_metric == 'dice':
            scores = DataStructs.BulkDiceSimilarity(base_fp, x_fps_list)
        elif distance_metric == 'cosine':
            scores = DataStructs.BulkCosineSimilarity(base_fp, x_fps_list)
        else:
            raise Exception("Please provide a valid distance metric (ie. euclidean, tanimoto, dice, cosine)")

        # Exclude the similarity to itself
        scores[i] = 1

        # Find the indices of the top N neighbors
        indices[i] = np.argpartition(scores, -n_neighbors)[-n_neighbors:]
        distances[i] = [scores[ind] for ind in indices[i]]
        
    return distances, indices

def tanimoto_distance(x, y, **kwargs):
    x_fp = DataStructs.CreateFromBitString(''.join(map(str, x)))
    y_fp = DataStructs.CreateFromBitString(''.join(map(str, y)))
    return 1 - DataStructs.TanimotoSimilarity(x_fp, y_fp)

"""
    Annotate the transfer data with the teacher models
---
            data : the FLuID dataset containing the molecules and their fingerprint
"""
# TODO: approximation is causing a seg fault error 
def annotate_transfer_data(transfer_data, teacher_models, teacher_data, params, force=False, distance_metric='euclidean', approximation=False):
    ...
    if not force and path.exists(pickleFile):
        ...
        for t in tqdm(range(0, k + 1)):
            ...
            # compute weights of teacher t based on 8 closest neighbours
            ...
            if approximation:
                nbrs = NNDescent(x, metric='rogerstanimoto', n_neighbors=8)
                indices, distances = nbrs.query(x, k=8)

            else:
                if distance_metric == 'tanimoto':
                    knn = NearestNeighbors(n_neighbors=8, metric=tanimoto_distance)
                else:
                    knn = NearestNeighbors(n_neighbors=8, metric=distance_metric)
            ...
            print("Size of transfer_data.index:", len(transfer_data.index))
        ...
        with open('full_distances_teachers.pickle', 'wb') as f1, open('full_indices_teachers.pickle', 'wb') as f2:
            pickle.dump(all_distances, f1)
            pickle.dump(all_indices, f2)
...
def benchmark(federated_data, teacher_models, test_data, params):
    ...
    student_validation_table = student_validation_table._append(
        pd.Series(['S-mean'], index=['Model'])._append(student_validation_table.iloc[1:].mean(axis=0)), ignore_index=True)
...
def cross_domain_validate(teacher_models, federated_data, test_data, params):
    if params['details'] > 2 :
        ...
        for teacher in tqdm(all_teachers):
            ...
            single_table = single_table._append(row, ignore_index=True)
            domain_table = domain_table._append(row, ignore_index=True)
            ...
            for other_teacher in all_teachers :
                if other_teacher != teacher:
                    ...
                    single_table = single_table._append(row, ignore_index=True)
                    domain_table = domain_table._append(row, ignore_index=True)
                    teacher_table = teacher_table._append(row, ignore_index=True)
