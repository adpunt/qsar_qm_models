...
# DNN-VBLL and MLP-VBLL × continuous_pdv × all 6 strategies — ANOVA only (no uncertainty)
# 2 models × 6 strategies = 12 runs, 10 bootstrap iterations each

for STRATEGY in legacy quantile threshold outlier heteroscedastic value_proportional; do
    for MODEL in dnn mlp; do
        echo "=== ${MODEL}_vbll / continuous_pdv / ${STRATEGY} ==="
