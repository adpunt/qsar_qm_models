...
# DNN-BNN Full and DNN-BNN Last × continuous_pdv × all 6 strategies — ANOVA only (no uncertainty)
...
for STRATEGY in legacy quantile threshold outlier heteroscedastic value_proportional; do
    for TRANSFORM in full last; do
        echo "=== dnn_bnn_${TRANSFORM} / continuous_pdv / ${STRATEGY} ==="
        python process_and_train.py -d QM9 -t homo_lumo_gap \
            ...
            -f ../results/anova_${STRATEGY}_continuous_pdv_dnn_bnn_${TRANSFORM}.csv
    done
done
