#!/bin/bash
#SBATCH --job-name=unc_cpdv_bnn_mlp
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# MLP-BNN Full and MLP-BNN Last × continuous_pdv × all 6 strategies — UNCERTAINTY (1 iteration)
...
for STRATEGY in legacy quantile threshold outlier heteroscedastic value_proportional; do
    for TRANSFORM in full last; do
        echo "=== mlp_bnn_${TRANSFORM} / continuous_pdv / ${STRATEGY} (uncertainty) ==="
        python process_and_train.py -d QM9 -t homo_lumo_gap \
            ...
            -f ../results/uncertainty_${STRATEGY}_continuous_pdv_mlp_bnn_${TRANSFORM}.csv
    done
done
