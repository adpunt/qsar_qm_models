#!/bin/bash
#SBATCH --job-name=morgan_unc_qrf
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# QRF × morgan (ECFP4 radius=2) × all 6 strategies — uncertainty extraction

for STRATEGY in legacy quantile threshold outlier heteroscedastic value_proportional; do
    echo "=== qrf / morgan / ${STRATEGY} (uncertainty) ==="
    python process_and_train.py -d QM9 -t homo_lumo_gap \
        ...
        -u True \
        --normalize True \
        -f ../results/uncertainty_${STRATEGY}_morgan_qrf.csv
done
