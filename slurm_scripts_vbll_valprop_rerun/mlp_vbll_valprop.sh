#!/bin/bash
#SBATCH --job-name=mlp_vbll_vp_rerun
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# MLP-VBLL × value_proportional (re-run: previous scripts used invalid strategy name "valprop")
for REP in ecfp4 pdv smiles mhggnn mol2vec; do
    echo "=== MLP-VBLL valprop ${REP} ==="
    python process_and_train.py -d QM9 -t homo_lumo_gap \
        ...
        -f ../results/anova_valprop_${REP}_mlp_bnn_variational.csv
done
